...
public class RpcMessageEncoder extends MessageToByteEncoder<RpcMessage> {
    ...
    protected void encode(ChannelHandlerContext ctx, RpcMessage rpcMessage, ByteBuf out){
        try {
            ...
            // 如果信息的类型不是心跳包的话总长度是fullLength = head length + body length
            ...
            int outwriteIndex=out.writerIndex();
            out.writerIndex(outwriteIndex-fullLength+RpcConstants.MAGIC_NUMBER.length+1);
            out.writeInt(fullLength);
            out.writerIndex(outwriteIndex);
        }catch (Exception e){
